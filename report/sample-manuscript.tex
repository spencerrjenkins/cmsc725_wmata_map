...
\section{Network Visualization and Evaluation}

Analysis and evaluation of the networks generated by these methods is done using a web application powered by Leaflet. The lines are shown over the metropolitan area map, with lines of the same group visualized in the same color. Visualization for the networks produced by each of the three methods can be selected. For visual comparison, I load the real-world transit network, which I am here defining as the existing and under-construction rail transport services as of 2025. These services, along with their type, length, number of lines and ridership, are provided in Table 2. I also provide the ability to view 500-meter station catchment areas for each line, helping aid the user to see what areas are best served by each station. The visualization app includes a route-finding feature, allowing the user to determine travel times and routes for the given network.
...
The genetic algorithm is the most sophisticated, and its output reflects this. There are strong connections between the urban core and suburbs in all directions

% DATA TO MAKE A TABLE FOR. The 16 numbers below are divided into four rows. The first row is Genetic, the second row is Iterative, the third row is Naive, and the final row is Real-world. In each row there are four data points. The columns are: Percent critical transit points covered, percent neighborhoods covered, average distance from closest station across metro area in meters, and average distance from closest station across District of Columbia in meters. Please round the percent values to the nearest hundredth of a percent, convert the distances to kilometers and round to the nearest hundredth, as requested.
% --- Table: Network Evaluation Metrics ---
...
\begin{tabular}{lcccc}
\toprule
\textbf{Network} & \textbf{\% Critical Points Covered} & \textbf{\% Neighborhoods Covered} & \textbf{Avg. Distance (km), Metro Area} & \textbf{Avg. Distance (km), DC} \\
\midrule
